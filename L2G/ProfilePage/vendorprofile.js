...
if (userData && userData.user_id) {
    ...
    supabase
        .from('vendor_data')
        .select('address, phone, shop_name')
        .eq('user_id', userId)
        .then(({ data, error }) => {
            if (error) {
                ...
                document.getElementById('shopname').textContent = 'Error fetching skills';
            } else if (data && data.length > 0) {
                const {  address, phone, shop_name } = data[0];
                ...
                document.getElementById('shopname').textContent = 'Skills not found';
            }
        });
} else {
    ...
    document.getElementById('shopname').textContent = 'User ID not found';
}
